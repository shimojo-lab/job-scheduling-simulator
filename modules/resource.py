...
class Resource:
    ...
    def print_nodes(self):
        ...
        for i, node in enumerate(self.nodes):
            ...
            print(f"Node {i}: {job_index} - {node.remaining_time} - {node.state}")
...
class Node:
    def __init__(self, timestep_seconds):
        ...
        self.scheduled_remaining_timesstep = 0
    ...
    def proceed_timestep(self):
        if self.state == NodeState.RUNNING:
            ...
            self.scheduled_remaining_timesstep -= 1
    ...
    def allocate_job(self, job: Job):
        ...
        self.scheduled_remaining_timesstep = np.ceil(
            job.real_time / self.timestep_seconds
        ).astype(int)
